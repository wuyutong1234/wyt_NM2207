...
**Solution:** "CSV" stands for "Comma Separated Value(s)". It is popular because it is a simple and universal format that can be easily read and written by many programs and tools, such as Python, R, SQL, and even text editors.
<br>
<br>
...
read_csv("~/Documents/NM2207/tutorial /Week-2/Challenge/playlist_data.csv") 
...
```
<br>
```{r, out.height= "400px",out.width= "800px",echo=TRUE,eval=TRUE,fig.cap="Screenshot of the information about read_csv"}
knitr::include_graphics("~/Documents/NM2207/tutorial /Week-2/infoabtcsv.png")
```
<br>

**Question 1.6:** What does the `skip` argument in the read_csv() function do?

**Solution:** The `skip` argument in the read_csv() function marks out the number of lines to skip before reading data. If comment is supplied any commented lines are ignored after skipping.
<br>
<br>
...
ggplot(data=playlist_data,mapping=aes(x=Age,y=Rating)) + geom_point()
...
ggplot(data=playlist_data,mapping=aes(x=Age,y=Rating)) + geom_point(shape=4) 
      ...
      labs(x="Age",
              y="Rating",
                title="Age versus Rating",
                  caption="Source: playlist_data.csv/ playlist dataset")
