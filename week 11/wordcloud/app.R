...
#renaming for easier display of x and y axis labels
...
create_plot <- function(data, x_var, y_var, title, custom_labels = NULL) {
  ggplot(data, aes_string(x = x_var, y = y_var)) +
    geom_point(color = "#43819d", size = 3) +
    geom_smooth(method = "lm", se = FALSE, color = "#daa671", linetype = "dashed", size = 1.2) +
    labs(
    ...
    theme_minimal() +
    theme(
      axis.text.x = element_text(color = "#245465", angle = 45, hjust = 1),  # Rotate x-axis labels
      plot.title = element_text(color = "#245465", hjust = 0.5, size = 16),  # Center and increase title size
      axis.title = element_text(color = "#245465", size = 14),  # Increase axis label size
